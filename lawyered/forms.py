...
class divorceForm(forms.Form):
    ...
    date_of_marriage = forms.DateField(initial=datetime.date,label = 'What was your Date of Marriage?')
...
class duiForm(forms.Form):
    date_of_citation = forms.DateField(initial = datetime.date , label = 'When did you receive the DUI citation?');
    ...
    next_date = forms.DateField(initial = datetime.now() + timedelta(days = 7), label = 'When is your next court date?')
...
class criminalForm(forms.Form):
    ...
    offense_date = forms.DateField(initial = datetime.date, label = 'What date did the incident occur?')
